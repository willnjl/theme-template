...
})()(
  {
    1: [
      function (require, module, exports) {
        ...
        userInstagram("fifteen.ten").then(console.log).catch(console.error);
      },
      { "user-instagram": 29 },
    ],
    2: [
      ...
      function (require, module, exports) {
        class UnableToFetchPostDataError extends Error {
          ...
          static fromShortcode(shortcode) {
            return new this(
              `An error occured while fetching post data for shortcode ${shortcode}. Maybe this post does not exist ?`
            );
          }
        }
      ...
      function (require, module, exports) {
        class UnableToFetchUserDataError extends Error {
          ...
          static fromUsername(username) {
            return new this(
              `An error occured while fetching data for user ${username}. Maybe this user does not exist ?`
            );
          }
        }
    ...
    32: [
                    ...
                    user.edge_owner_to_timeline_media.edges.map((edge) => {
                      ...
                      return {
                        ...
                        children: edge.node.edge_sidecar_to_children
                          ? edge.node.edge_sidecar_to_children.edges.map(
                              (edge) => {
                                return {
                                  id: edge.node.id,
                                  shortCode: edge.node.shortcode,
          ...
          return new Promise(async (resolve, reject) => {
            ...
            axios(REQUEST_PARAMETERS)
              .then((GQL) => {
                ...
                const has_caption =
                  media_data.edge_media_to_caption.edges.length > 0;
                resolve({
                  link: URL.replace("/?__a=1", ""),
                  ...
                  childrenPictures:
                    media_data.edge_sidecar_to_children &&
                    media_data.edge_sidecar_to_children.edges
                      ? media_data.edge_sidecar_to_children.edges.map(
                          (edge) => {
                            return {
                              id: edge.node.id,
                              shortcode: edge.node.shortcode,
                      ...
                      : [],
                  comments: media_data.edge_media_to_parent_comment.edges.map(
                    (edge) => {
                      return {
                        id: edge.node.id,
                        text: edge.node.text,
      ...
      {
        "../src/util": 33,
        "./Exceptions/UnableToFetchPostDataError": 30,
        "./Exceptions/UnableToFetchUserDataError": 31,
        axios: 2,
      },
    ],
    33: [
